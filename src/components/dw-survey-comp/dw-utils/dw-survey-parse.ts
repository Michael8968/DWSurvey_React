 ...
 * @param survey
 */
export function parseSurvey(survey: any) {
  /* survey: {
  ...
  if (survey !== null) {
    if (!survey.hasOwnProperty("surveyNameObj"))
      survey.surveyNameObj = {
    ...
    if (!survey.surveyStyle.hasOwnProperty("showQuScoreNum"))
      survey.surveyStyle.showQuScoreNum = false;
...
export function parseSurveyDetail(survey: any) {
  const surveyDetail = survey.surveyDetail;
  if (surveyDetail !== null) {
    const surveyNoteText =
      surveyDetail.surveyNoteText !== null ? surveyDetail.surveyNoteText : "";
    // surveyDetail.surveyNodeObj = {dwHtml: surveyDetail.surveyNote, dwText: surveyNoteText}
    if (!survey.surveyDetail.hasOwnProperty("surveyNodeObj"))
      survey.surveyDetail.surveyNodeObj = {
        dwHtml: surveyDetail.surveyNote,
        dwText: surveyNoteText,
        dwPlaceholder: "请输入问卷介绍",
      };
  }
  ...
  if (!survey.surveyDetail.hasOwnProperty("dwId"))
    survey.surveyDetail.dwId = uuidv4();
  // v6版本重新定义问卷属性存放类
 ...
 * @param questions
 */
export function parseQuestions(questions: any[], noModel: boolean) {
 ...
 * @param noModel
 */
export function parseQuestion(question: any, noModel: boolean) {
  const quName = question.quName !== null ? question.quName : question.quTitle;
  if (!question.hasOwnProperty("quTitleObj"))
    question.quTitleObj = {
      dwHtml: question.quTitle,
      dwText: quName,
      dwPlaceholder: "请输入题目标题",
      isNew: false,
    };
  const quNote = question.quNote;
  if (!question.hasOwnProperty("quNoteObj"))
    question.quNoteObj = {
      dwHtml: quNote,
      dwText: quNote,
      dwPlaceholder: "请输入题目备注",
      isNew: false,
    };
  if (
    !question.hasOwnProperty("questionLogics") ||
    question.questionLogics === null
  )
  ...
  if (!question.hasOwnProperty("validateObj"))
...
function addNewQuProps(question: any) {
  // 新版重新梳理属性结构
  if (question.hasOwnProperty("quAttr")) {
    if (!question.quAttr.hasOwnProperty("isRequired"))
  ...
  if (
    quType === "CHECKBOX" &&
    !question.quAttr.scoreAttr.hasOwnProperty("allRight")
  ) {
...
function getInputQuProps() {
  ...
  const dateTimeAttr = {
    timeRange: { range: null, step: null },
    dateFormat: null,
    attrs: [],
  };
  const numAttr = { min: null, max: null };
 ...
 * @returns {*}
 */
export function initQuestionModels(questions: any[]) {
  if (questions !== null && questions.length > 0) {
    // 循环然后定义以上内容
 ...
 * @param question
 */
function parseQuRadio(question: any) {
 ...
 * @param question
 */
function parseQuCheckbox(question: any) {
  question.quTypeName = "多选题";
  if (question.cellCount === 0) question.cellCount = 2;
  parseQuOptionType1(question, question.quCheckboxs);
  if (!question.quAttr.hasOwnProperty("scoreAttr")) {
    question.quAttr.scoreAttr = {
      maxScore: 0,
      designShowScoreNum: false,
      allRight: { enabled: false, scoreNum: 0 },
    };
  }
}
 ...
 * @param question
 */
function parseQuOrderbys(question: any) {
 ...
 * @param question
 */
function parseQuMultiFillblanks(question: any) {
 ...
 * @param question
 */
function parseQuScores(question: any) {
 ...
 * @param question
 */
function parseQuFbk(question: any) {
  ...
  addNewQuProps(question);
  if (
    !question.quAttr.hasOwnProperty("inputAttr") ||
    question.quAttr.inputAttr === undefined
  )
    question.quAttr.inputAttr = inputAttr;
  if (!question.quAttr.inputAttr.hasOwnProperty("dateTimeAttr"))
    question.quAttr.inputAttr.dateTimeAttr = inputAttr.dateTimeAttr;
...
function parseQuUploadFile(question: any) {
 ...
 * @param quOptions
 */
function parseQuOptionType1(question: any, quOptions: any[]) {
  if (quOptions !== null && quOptions.length > 0) {
    quOptions.forEach((quOption, optionIndex) => {
      if (question.hasOwnProperty("dwId") && !quOption.hasOwnProperty("dwId"))
...
export function parseQuOptionTypeByQu(question: any, quOption: any) {
  const quType = question.quType;
  if (
    quType === "RADIO" ||
    quType === "CHECKBOX" ||
...
export function parseQuOptionType1Item(quOption: any) {
  const optionTitle =
    quOption.optionTitle !== null ? quOption.optionTitle : quOption.optionName;
  // oss版本把html保存在 optionTitle
  const optionName =
    quOption.optionName !== null ? quOption.optionName : optionTitle;
  if (!quOption.hasOwnProperty("optionTitleObj"))
    quOption.optionTitleObj = {
      dwHtml: optionName,
      dwText: optionTitle,
 ...
 * @param question
 */
export function resetQuestion(question: any) {
 ...
 * @param quOptions
 */
function resetQuOptionType1(question: any, quOptions: any[]) {
  if (quOptions !== null && quOptions.length > 0) {
    quOptions.forEach((quOption, optionIndex) => {
      quOption.dwId = uuidv4();
    });
 ...
 * @param quCols
 */
function resetMatrixQuOptionType1(question: any, quRows: any[], quCols: any[]) {
 ...
 * @param question
 */
export function buildMatrixOption(question: any) {
  const quType = question.quType;
  if (
    quType === "MATRIX_RADIO" ||
    quType === "MATRIX_CHECKBOX" ||
    ...
    if (
      !question.hasOwnProperty("quRows") ||
      question.quRows === null ||
      question.quRows.length === 0
    ) {
      const quRows = [];
      for (let i = 0; i < 3; i++) {
        const quOption: any = {
  ...
  if (
    ...
    if (
      !question.hasOwnProperty("quCols") ||
      question.quCols === null ||
      question.quCols.length === 0
    ) {
      const quCols = [
        {
          dwId: uuidv4(),
          optionTitleObj: { dwHtml: "", dwText: "", dwPlaceholder: "" },
          scoreNum: null,
          tempEmptyOption: true,
        },
      ]; // tempEmptyOption 表示不是真实选项，用于显示列时保证列数多一列。
      for (let i = 0; i < 3; i++) {
        const quOption = {
          dwId: uuidv4(),
          optionTitleObj: {
