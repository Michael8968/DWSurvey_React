 ...
 */
export function getSurveyJsonBySurveyId (params: any, successCallback: any, noJsonCallback: any) {
...
export function getDesignSurveyJsonBySurveyId (params: any, successCallback: any) {
  // 先看看有没有JSON，有就取JSON数据。没有再取原来的Survey结构数据进行转换
 ...
 * @param successCallback
 */
export function getQuerySurvey (params: any, successCallback: any) {
...
export const surveyPageUtils= {
  ...
  quInPageNum (survey: any, quIndex: number) {
    const questions = survey.questions
    let pageNum = 0
    questions.forEach((item: any, index: number) => {
...
export function getSurveyTypeSimpleName (survey: any) {
  if (survey!==null) {
