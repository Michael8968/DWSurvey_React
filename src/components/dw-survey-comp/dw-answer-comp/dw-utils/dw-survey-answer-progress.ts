...
export function initAnswerSurveyProgress (survey: any) {
  // 如果没有进度信息的情况下执行初始化
  if (!survey.hasOwnProperty('answerProgress')) {
    answerSurveyProgressCommon(survey, (question: any) => {
...
export function answerSurveyProgress (survey: any) {
  answerSurveyProgressCommon(survey, (question: any) => question.validateObj.isAnswerOk)
}

function answerSurveyProgressCommon (survey: any, quIsAnOkFun: any) {
  ...
  if (answerProgress.totalAnQu>0) {
    answerProgress.percentage = Number((answerProgress.completeAnQu/answerProgress.totalAnQu * 100).toFixed(2))
  }
...
function continueQuType (question: any) {
